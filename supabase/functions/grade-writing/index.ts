...
async function callAIWithRetry(prompt: string, apiKey: string, retries = 2): Promise<string> {
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`AI attempt ${i + 1} failed:`, response.status, errorText);
        if (i === retries) throw new Error(`AI error: ${response.status}`);
  ...
  throw new Error("All AI retries failed");
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!attempt_id || !question_id) {
      throw new Error("Missing attempt_id or question_id");
    }
...
Be thorough but fair. If the student's text is empty or nonsensical, give a score of 0.`;
    ...
    const content = await callAIWithRetry(prompt, LOVABLE_API_KEY);
    const jsonStr = content.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
    const feedback = JSON.parse(jsonStr);

    const { error: updateError } = await supabase.from("answers").update({
